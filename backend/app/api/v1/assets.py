...
def get_asset_type_from_file(file: UploadFile) -> str:
    ...
    if mime:
        ...
        if 'spreadsheet' in mime or 'excel' in mime: return 'spreadsheet'
        if 'presentation' in mime or 'powerpoint' in mime: return 'presentation'

    # --- Fallback check: File Extension (for when MIME is generic) ---
    if extension:
        if extension in ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg']: return 'image'
        if extension == 'pdf': return 'pdf'
        if extension == 'csv': return 'csv'
        if extension == 'md': return 'markdown'
        if extension == 'txt': return 'text'
        if extension in ['xls', 'xlsx']: return 'spreadsheet'
        if extension in ['ppt', 'pptx']: return 'presentation'
...
async def delete_asset(
    ...
    if not success:
        ...
        logger.warning(f"Could not delete file from storage for asset {asset_id}, but proceeding to delete metadata.")
