...
class CraftedContent(BaseModel):
    title: Optional[str] = Field(
        None, description="A main title for a slide or document."
    )
    subtitle: Optional[str] = Field(
        None, description="A subtitle that complements the main title."
    )
    bullet_points: Optional[List[str]] = Field(
        None, description="A list of key points, suitable for a content slide."
    )
    body_text: Optional[str] = Field(None, description="A paragraph of detailed text.")
    image_search_query: Optional[str] = Field(
        None,
        description="A concise search query for a relevant image (e.g., 'futuristic AI robot').",
    )
