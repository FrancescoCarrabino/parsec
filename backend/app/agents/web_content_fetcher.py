...
class WebContentFetcher(Agent):
    ...
    """
    @property
    def name(self) -> str: return "WebContentFetcher"
    ...
    async def run_task(
        ...
        if not url or not url.startswith(('http://', 'https://')):
            error_msg = f"Invalid URL provided: '{url}'. URL must be a valid web address."
        ...
        try:
            ...
            await send_status_update("INVOKING_TOOL", f"Using the tool _fetch_and_clean_url_content...", {"target_tool": "_fetch_and_clean_url_content"})
            ...
            return {"status": "failed", "error": f"An error occurred while fetching web content: {str(e)}"}
