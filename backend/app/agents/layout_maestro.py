...
class LayoutMaestro(Agent):
                ...
                function={
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            "spacing": {
                                "type": "number",
                                "description": "The gap in pixels between each element.",
                            },
                            "direction": {
                                "type": "string",
                                "enum": ["horizontal", "vertical"],
                                "description": "The direction to apply the spacing.",
                            },
                        },
    ...
    async def run_task(
        ...
        if not element_ids or len(element_ids) < 2:
            return {
                "status": "failed",
                "error": "LayoutMaestro requires at least two selected elements.",
            }
        ...
        **CRITICAL INSTRUCTIONS:**
        ...
        try:
            ...
            if not response_message.tool_calls:
                return {
                    "status": "failed",
                    "error": f"{self.name} could not determine a layout action from the objective.",
                }
            ...
            for tool_call in response_message.tool_calls:
                ...
                if not tool_function:
                    logger.error(
                        f"LayoutMaestro received unimplemented tool call: {tool_name}"
                    )
                    continue  # Skip to next tool call if one is bad

                await send_status_update(
                    "INVOKING_TOOL",
                    f"Using the tool {tool_function}...",
                    {"target_tool": tool_name},
                )
                await tool_function(
                    context=context, element_ids=element_ids, **tool_args
                )
    ...
    async def _calculate_and_apply_alignment(
        self, context: dict, element_ids: List[str], alignment: str
    ) -> None:
        elements = [
    ...
    async def _calculate_and_apply_distribution(
        self, context: dict, element_ids: List[str], direction: str
    ) -> None:
        elements = [
            self._workspace.elements.get(eid)
            for eid in element_ids
        ...
        if len(elements) < 3:
            return  # Distribution needs at least 3 elements
    ...
    async def _calculate_and_apply_spacing(
        self, context: dict, element_ids: List[str], spacing: float, direction: str
    ) -> None:
        elements = [
            self._workspace.elements.get(eid)
            for eid in element_ids
