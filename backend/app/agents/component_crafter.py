...
class ComponentCrafter(Agent):
    ...
    @property
    def name(self) -> str: return "ComponentCrafter"
    ...
    def tools(self) -> List[Tool]:
        return [
            Tool(function={
                ...
                "parameters": {
                    "type": "object", "properties": {"element_ids": {"type": "array", "items": {"type": "string"}}}, "required": ["element_ids"]
                }
            }),
            Tool(function={
                ...
                "parameters": {
                    "type": "object",
                    "properties": {
                        "name": {"type": "string", "description": "A descriptive name for the new component (e.g., 'Primary Button', 'User Profile Card')."},
                        "source_element_ids": {"type": "array", "items": {"type": "string"}, "description": "The original element IDs to be converted into the component."},
                        "schema": {
                            "type": "array",
                            "items": {
                                "type": "object",
                                "properties": {
                                    "prop_name": {"type": "string", "description": "A machine-readable name for the property (e.g., 'button_label', 'user_avatar_src')."},
                                    "target_element_id": {"type": "string", "description": "The ID of the element within the component that this property controls."},
                                    "target_property": {"type": "string", "description": "The attribute of the target element to change (e.g., 'content' for text, 'src' for images)."},
    ...
    async def run_task(self, objective: str, context: Dict[str, Any], invoke_agent: Callable, send_status_update: Callable) -> Dict[str, Any]:
        ...
        if not element_ids:
            return {"status": "failed", "error": "ComponentCrafter requires at least one element to be selected."}
        ...
        """
        messages = [{"role": "system", "content": system_prompt}, {"role": "user", "content": f"Objective: '{objective}'. Selected element IDs are: {json.dumps(element_ids)}"}]
        ...
        try:
            ...
            for _ in range(3): # Limit to 3 steps: inspect -> define -> finish
                await send_status_update("THINKING", "Analyzing selected elements...", {"agent_name": self.name})
                ...
                for tool_call in response_message.tool_calls:
                    ...
                    await send_status_update("INVOKING_TOOL", f"Using tool: {tool_name}...", {"target_tool": tool_name})
                    tool_function = self.available_functions.get(tool_name)
                    tool_result = await tool_function(context=context, **tool_args)
                    messages.append({"role": "tool", "tool_call_id": tool_call.id, "name": tool_name, "content": json.dumps(tool_result)})
    ...
    async def _get_elements_by_id(self, context: dict, element_ids: List[str]) -> Dict[str, Any]:
        ...
        summary = [
            {
                "id": el.id, "element_type": el.element_type,
    ...
    async def _define_component_from_elements(self, context: dict, name: str, source_element_ids: List[str], schema: List[Dict]) -> Dict[str, Any]:
        new_def, new_inst, deleted_ids = self._workspace.create_component_from_elements(name, source_element_ids, schema)
        if not new_def or not new_inst:
            return {"status": "failed", "error": "Workspace service failed to create the component."}

        # Append all necessary commands to the main workflow context
        context["commands"].append({"type": "COMPONENT_DEFINITION_CREATED", "payload": new_def.model_dump()})
        context["commands"].append({"type": "ELEMENT_CREATED", "payload": new_inst.model_dump()})
        for an_id in deleted_ids:
            context["commands"].append({"type": "ELEMENT_DELETED", "payload": {"id": an_id}})
            
        return {"status": "success", "definition_id": new_def.id, "instance_id": new_inst.id}